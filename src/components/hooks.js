  ...
  useEffect(() => {
    try {
      const run = async () => {
        ...
        //const blockDetails1 = await provider.getBlock('latest')
        //console.log('blockDetails',blockDetails1)
        const blockDetails = await provider.getGasPrice()
        console.log('blockDetails',blockDetails)
        if (blockDetails) {
          console.log(blockDetails._hex)
          let baseFeeWei = ethers.utils.formatUnits(
            Number(blockDetails._hex),
            'wei'
          )
          baseFeeWei = baseFeeWei ;
          console.log('baseFeeWei',baseFeeWei)
          const price = {
      ...
      const run = async () => {
        try{
          const result = await fetch(
            `https://metadata.fantomdomains.io/${_network}/avatar/${name}/meta`
          )
