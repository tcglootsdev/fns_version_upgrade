...
const TabLink = styled(Link)`
  font-size: 14px;
  background: ${({ active }) => (active=='true' ? '#2C46A6' : 'transparent')};
  color: ${({ active }) => (active=='true' ? 'white' : '#D2D2D2')};
  transform: scale(${({ active }) => (active=='true' ? '1.08' : '1')});
  ...
  return (
    (state !== 'Auction' || state !== 'Reveal') && (
      <TabContainer>
          ...
          <TabLink
            active={
              ...
              (pathname !== `/name/${name}/details` &&
                pathname !== `/name/${name}/subdomains`)).toString()
            }
        ...
        <TabLink
          active={getDetailsActive(domain, pathname, tab).toString()}
          to={`/name/${name}/details`}
        >
          {t('singleName.tabs.details')}
        </TabLink>
        <TabLink
          active={(pathname === `/name/${name}/subdomains`).toString()}
